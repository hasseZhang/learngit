...
var LEFT_LIST = (function() {
    ...
    var _listId = G_GUIDEID.local.left; // 左侧列表ID
    var _data = [];
    var _position = 0;
    var _dataLen = 0;
    var _saveData = {
...
var RIGHT_LIST_T = (function() {
    ...
    function _ok() {
        ...
        if (type == 'battleSummary') {
            ...
            var url = $$.search.append(window.location.href.replace('local/', 'battleData/'), { 'scheduleId': scheduleId });
            $$.forward(url);
        } else {
            var teamId = data.teamID;
            var playerId = data.playerID;
            var url = $$.search.append(window.location.href.replace('local/', 'starDetail/'), { 'teamId': teamId, 'starId': playerId });
            $$.forward(url)
        }
    }
