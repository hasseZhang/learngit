...
function getNowModule() {
    return {
        ...
        renderHtml: function(data, begin) {
            var src = "", title = "", className = "", html = "";
            for (var i = 0; i < data.length; i++) {
                className = "poster-name autoText";
                src = $.getPic(data[i].pics, [ 6 ]);
                title = data[i].contentName;
                html += '<div class="list_item" id="list_item' + (begin + i) + '"><div class="poster noContent"><img src="' + src + '"></div><div class="' + className + '">' + title + "</div></div>";
            }
            return html;
        },
        active: function() {
