...
function load() {
    vl = $.createVideoList({
            ...
            if (type === "stream") {
                ...
                if(isPreVideo){
                    ...
                    vl.mp.once($.MP.state.loaded, function(param) {
                        ...
                        $.vs.xljPlay(vl.current().contentId, totalTime, (page.isTry ? "TRY-" : "") + vl.current().sceneName, page.categoryId, $.auth.getchargeSpIds(page.seriesId), page.ztCategoryId);
                        if ($.pTool.get("p_trySee") && $.pTool.get("p_trySee").getInfo().isShow) {
                            return;
                        }
                        if (isShowHisCue) {
                ...
                $.pTool.get("progress").setInfo({
                    videoName: listData[playIndex].sceneName,
                    isTry: page.isTry,
                    hisCueIsShow: hisCue.getIsShow,
                    // isPlayAd: isPlayAd
                });
                $.pTool.get("xiliejuList").initPlayer(vl.mp);
                $.pTool.get("xiliejuList").setInfo({
        ...
        onEnd: function(current, info, isPreVideo) {
            ...
            if (page.isTry) {
                ...
                noBuyCurrent++;
                if (noBuyCurrent > listData.length - 1) {
                    noBuyCurrent = 0;
                }
                vl.seek({
                    val: noBuyCurrent,
    ...
    $.pTool.get("progress").setInfo({
        videoName: vl.current().sceneName,
        isTry: page.isTry,
        // isPlayAd:isHasAd
    });
    $.pTool.get("xiliejuList").initPlayer(vl.mp);
        ...
        if (vl) {
            ...
            if (isCanSaveHis && vl.mp) {
                ...
                $.s.his.add({
                    ...
                    sceneInfo: listData[saveIndex].latestTime || listData.length - saveIndex,
                    totalTime: totalTime
                });
            }
        ...
        show: function(time) {
            ...
            if (page.isTry) {
                ...
                if (playIndex < listData.length - 1) {
                    endText = "即将播放: " + listData[playIndex + 1].sceneName;
                    $tip.addClass("normal");
                } else {
                    $tip.removeClass("normal");
...
$.pTool.add("xiliejuList", function() {
    var key = "xiliejuList";
    var autoHide;
    var $content = null;
    var playingIndex = 0;
    ...
    function list() {
        ...
        for (var i = 0; i < num1; i++) {
            var newTag=data.listData[i]&&data.listData[i].sceneLatestStatus?data.listData[i].sceneLatestStatus:null;
            $('<div id="list' + i + '"><div id="mar" class="marquee">' + data.listData[focus.nav * listLength + i].sceneName + "</div></div>").appendTo(".list");
            // if (data.vipFlag) {
            ...
            var nav = 0;
            var index = 0;
            if (playingIndex >= listLength - 1) {
                if ((playingIndex + 1) % listLength == 0) {
                    nav = parseInt(playingIndex / listLength);
                    index = listLength - 1;
                } else {
                    nav = parseInt(playingIndex / listLength);
                    index = playingIndex % listLength;
                }
            } else {
                nav = 0;
                index = playingIndex;
    ...
    function reset() {
        ...
        if (playingIndex >= listLength - 1) {
            if ((playingIndex + 1) % listLength == 0) {
                focus.nav = parseInt(playingIndex / listLength);
                focus.index = listLength - 1;
            } else {
                focus.nav = parseInt(playingIndex / listLength);
                focus.index = playingIndex % listLength;
            }
        } else {
            focus.nav = 0;
            focus.index = playingIndex;
        }
    }
    function pressLeft() {
        if (isPlayAd) {
    ...
    function playOrOrder(playIndex) {
        ...
        if (data.listData[playIndex].sceneTimes <= 30 && data.vipFlag) {
            gotoOrder();
            return true;
        }
    ...
    return {
        ...
        init: function(opt) {
            ...
            $content = $('<div class="xiliejuContent"></div>').appendTo("body");
            $('<div class="title">选期</div>').appendTo($content);
            ...
            for (var i = 0; i < num; i++) {
                var num1 = data.totalNum - listLength * i;
                var num2 = data.totalNum - (listLength * (i + 1) - 1);
                var tag = false;
                num2 = num2 < 1 ? 1 : num2;
                var content = num1 == num2 ? num1 : num1 + "-" + num2;
                for(var h=num2-1;h<num1;h++){
                    if(data.listData[h]&&data.listData[h].sceneLatestStatus&&data.listData[h].sceneLatestStatus=="1"){
                        tag = true;
                    }
                }
                if(num>1&&tag){
